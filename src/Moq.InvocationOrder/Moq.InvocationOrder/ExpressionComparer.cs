...
namespace Moq.InvocationOrder
{
    public sealed class ExpressionComparer : IEqualityComparer<Expression>
    {
                ...
                case ExpressionType.Constant:
                    {
                        switch (this.CompareConstants)
                        {
                            ...
                            case ConstantComparison.ByExpressionReference:
                                return x == y;
                            default:
                                throw new InvalidEnumArgumentException(nameof(CompareConstants), (int)this.CompareConstants, typeof(ConstantComparison));
                        }
                    }
        ...
        private static bool CompareNames(string name1, string name2, NameComparison comparison)
        {
            switch (comparison)
            {
                ...
                default:
                    throw new InvalidEnumArgumentException(nameof(comparison), (int)comparison, typeof(NameComparison));
            }
        }
